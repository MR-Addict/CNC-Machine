...
void report_status_message(uint8_t status_code)
{
  ...
  else
  {
    ...
    case STATUS_IDLE_ERROR:
      printPgmString(PSTR("Not idle"));
      break;
    case STATUS_ALARM_LOCK:
      printPgmString(PSTR("Alarm lock"));
      break;
    case STATUS_SOFT_LIMIT_ERROR:
      printPgmString(PSTR("Homing not enabled"));
      break;
    ...
    case STATUS_MAX_STEP_RATE_EXCEEDED:
      printPgmString(PSTR("Step rate > 30kHz"));
      break;
#endif
    ...
    case STATUS_GCODE_UNSUPPORTED_COMMAND:
      printPgmString(PSTR("Unsupported command"));
      break;
    case STATUS_GCODE_UNDEFINED_FEED_RATE:
      printPgmString(PSTR("Undefined feed rate"));
      break;
    default:
...
void report_alarm_message(int8_t alarm_code)
{
  ...
  case ALARM_SOFT_LIMIT_ERROR:
    printPgmString(PSTR("Soft limit"));
    break;
...
// Print Grbl settings.
  ...
  for (set_idx = 0; set_idx < AXIS_N_SETTINGS; set_idx++)
  {
    for (idx = 0; idx < N_AXIS; idx++)
    {
      ...
      {
      case 0:
        printPgmString(PSTR(", step/mm"));
        break;
      case 1:
        printPgmString(PSTR(" max rate, mm/min"));
        break;
      case 2:
        printPgmString(PSTR(" accel, mm/sec^2"));
        break;
      case 3:
        printPgmString(PSTR(" max travel, mm"));
        break;
      }
      printPgmString(PSTR(")\r\n"));
#endif
...
void report_realtime_status()
{
  ...
  if (bit_istrue(settings.status_report_mask, BITFLAG_RT_STATUS_WORK_POSITION))
  {
    ...
    for (idx = 0; idx < N_AXIS; idx++)
    {
      ...
      {
        print_position[idx] -= gc_state.tool_length_offset;
      }
      printFloat_CoordValue(print_position[idx]);
      if (idx < (N_AXIS - 1))
      {
